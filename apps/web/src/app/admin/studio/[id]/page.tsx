    ...
    return (
      <div className="min-h-screen bg-canvas flex items-center justify-center p-6">
        <div className="bg-surface border border-line rounded-2xl p-8 text-center max-w-sm">
          ...
          <h1 className="font-bold text-ink">Erişim yok</h1>
          <Link href="/admin" className="inline-flex items-center gap-2 mt-5 text-brand font-semibold"><ArrowLeft className="w-4 h-4" /> Panele dön</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-canvas text-ink">
      <header className="sticky top-0 z-20 bg-surface border-b border-line">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <Link href="/admin" className="text-ink-muted hover:text-brand shrink-0"><ArrowLeft className="w-5 h-5" /></Link>
            <div className="min-w-0">
              <p className="font-bold text-sm truncate">{tenant.name}</p>
              <p className="text-ink-subtle text-xs truncate">/{tenant.slug} · {tenant.is_active ? "Aktif" : "Pasif"}</p>
            </div>
          </div>
          <a href={`/book/${tenant.slug}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-sm font-medium text-ink-muted hover:text-brand shrink-0">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 sm:gap-4">
          {KPIS.map((k) => (
            <div key={k.label} className="bg-surface border border-line rounded-2xl p-4">
              ...
              <p className="text-ink-subtle text-xs mt-0.5">{k.label}</p>
            </div>
          ))}
        </div>
              ...
              {monthly.map((m: any) => (
                <div key={m.label} className="flex-1 flex flex-col items-center gap-1.5">
                  <div className="w-full flex items-end justify-center" style={{ height: 120 }}>
                    <div
                      className="w-full max-w-[34px] rounded-t-lg bg-brand/80"
                      style={{ height: `${(m.revenue / maxRev) * 100}%`, minHeight: m.revenue > 0 ? 4 : 0 }}
                      title={formatPrice(m.revenue, "TRY")}
                    />
                  </div>
                  <span className="text-[10px] text-ink-subtle">{m.label}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              {Object.keys(STATUS_LABELS).map((s) => {
                ...
                return (
                  <div key={s} className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-ink-muted w-8 text-right">{count}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="bg-surface border border-line rounded-2xl p-5">
          ...
          {staff.length === 0 ? (
            <p className="text-ink-subtle text-sm">Personel yok.</p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {staff.map((m: any) => (
                <div key={m.id} className="flex items-center gap-2 bg-canvas border border-line rounded-xl px-3 py-2">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center text-white text-xs font-bold shrink-0" style={{ backgroundColor: m.color ?? "rgb(var(--ns-brand))" }}>
                    {m.display_name?.charAt(0)}
                  </div>
                  <span className="text-sm">{m.display_name}</span>
                  {!m.is_active && <span className="text-[10px] text-ink-subtle">(pasif)</span>}
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-surface border border-line rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-line"><h2 className="font-semibold">Son Randevular</h2></div>
          {recent.length === 0 ? (
            <p className="text-ink-subtle text-sm px-5 py-6">Henüz randevu yok.</p>
          ) : (
                ...
                <tbody className="divide-y divide-line">
                  {recent.map((a: any) => (
                    <tr key={a.id} className="hover:bg-canvas">
                      ...
                      <td className="px-5 py-3 text-right text-brand font-semibold whitespace-nowrap">{formatPrice(a.final_price, "TRY")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
