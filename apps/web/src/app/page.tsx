  ...
  return (
    <div className="min-h-screen bg-canvas text-ink">
        ...
        <nav className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5">
            ...
            <div className="leading-tight">
              ...
              <p className="text-brand text-xs font-medium -mt-0.5">101</p>
            </div>
          </Link>
          <div className="flex items-center gap-2 sm:gap-3">
            <Link href="#fiyatlandirma" className="hidden sm:block text-sm font-medium text-ink-muted hover:text-brand px-3 py-2 transition-colors">
              Fiyatlar
            </Link>
            <Link href="/hesabim" className="hidden sm:block text-sm font-medium text-ink-muted hover:text-brand px-3 py-2 transition-colors">
              Randevularım
            </Link>
            <Link href="/auth/login" className="text-sm font-medium text-ink-muted hover:text-brand px-3 py-2 transition-colors">
              Giriş Yap
            </Link>
            <Link href="/auth/register" className="text-sm font-semibold bg-brand hover:bg-brand-dark text-white px-4 py-2 rounded-xl transition-colors shadow-lg shadow-brand/20">
              Ücretsiz Başla
            </Link>
          </div>
        </nav>
      ...
      <section className="max-w-6xl mx-auto px-5 pt-16 pb-20 sm:pt-24 sm:pb-28 text-center">
        <span className="inline-flex items-center gap-1.5 bg-brand-soft border border-brand/20 text-brand text-xs font-semibold px-3 py-1.5 rounded-full mb-6">
        ...
        <h1 className="text-4xl sm:text-6xl font-bold tracking-tight leading-[1.1]">
          ...
          <span className="text-brand">akıllı randevu</span> sistemi
        </h1>
        ...
        <div className="mt-9 flex flex-col sm:flex-row items-center justify-center gap-3">
          <Link href="/auth/register" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-brand hover:bg-brand-dark text-white font-semibold px-7 py-3.5 rounded-2xl transition-all shadow-xl shadow-brand/25 active:scale-95">
            Ücretsiz Başla <ArrowRight className="w-4 h-4" />
          </Link>
          <Link href="#fiyatlandirma" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-surface border border-line hover:border-brand text-ink font-semibold px-7 py-3.5 rounded-2xl transition-all">
            Fiyatlandırma
          </Link>
        </div>
        <p className="mt-4 text-xs text-ink-subtle">Kredi kartı gerekmez · Dakikalar içinde kurulum</p>
      </section>
      ...
      <section className="bg-surface border-y border-line">
        <div className="max-w-6xl mx-auto px-5 py-20">
          <div className="text-center mb-14">
            ...
            <p className="mt-3 text-ink-muted">Dağınık defterleri ve karışık mesajları geride bırakın.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {FEATURES.map((f) => (
              <div key={f.title} className="bg-canvas border border-line rounded-2xl p-6 hover:border-brand/40 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {STEPS.map((s) => (
            <div key={s.n} className="text-center">
              <div className="w-12 h-12 rounded-2xl bg-brand text-white font-bold text-lg flex items-center justify-center mx-auto shadow-lg shadow-brand/25">
                {s.n}
              </div>
              <h3 className="mt-5 font-semibold text-lg">{s.title}</h3>
              <p className="mt-1.5 text-sm text-ink-muted max-w-xs mx-auto">{s.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Pricing */}
      <section id="fiyatlandirma" className="scroll-mt-20 bg-surface border-y border-line">
        <div className="max-w-6xl mx-auto px-5 py-20">
          <div className="text-center mb-14">
            <h2 className="text-3xl sm:text-4xl font-bold">Basit, şeffaf fiyatlandırma</h2>
            <p className="mt-3 text-ink-muted">Stüdyona uygun planı seç, istediğin zaman yükselt.</p>
          </div>
            ...
            {PLANS.map((p) => (
              <div
                ...
                className={`relative rounded-3xl p-7 border ${
                  p.highlight
                    ? "border-brand bg-brand-soft shadow-xl shadow-brand/15 md:-mt-3"
                    : "border-line bg-surface"
                }`}
              >
                {p.highlight && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-brand text-white text-xs font-semibold px-3 py-1 rounded-full shadow-lg shadow-brand/30 whitespace-nowrap">
                    En Popüler
                  </span>
                )}
                <h3 className="font-bold text-lg">{p.name}</h3>
                <p className="text-sm text-ink-muted mt-1 min-h-[40px]">{p.desc}</p>
                <div className="mt-4 flex items-end gap-1">
                  <span className="text-4xl font-bold">{p.price}</span>
                  <span className="text-ink-subtle text-sm mb-1.5">{p.period}</span>
                </div>
                <Link
                  href="/auth/register"
                  className={`mt-6 w-full inline-flex items-center justify-center gap-2 font-semibold px-5 py-3 rounded-xl transition-all active:scale-95 ${
                    p.highlight
                      ...
                      : "bg-surface border border-line hover:border-brand text-ink"
                  }`}
                >
                  {p.cta}
                </Link>
      ...
      <section className="max-w-6xl mx-auto px-5 pb-24">
        <div className="bg-brand rounded-3xl px-8 py-14 sm:py-16 text-center text-white shadow-2xl shadow-brand/30">
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link href="/auth/register" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-surface text-brand font-semibold px-7 py-3.5 rounded-2xl hover:bg-white/90 transition-all active:scale-95">
              Ücretsiz Başla <ArrowRight className="w-4 h-4" />
            </Link>
